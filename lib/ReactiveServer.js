...
class ReactiveServerConnection {
  ...
  constructor(server, id, connection, daoFactory, settings) {
    ...
    this.context = null
    this.healthChecker = this.settings.healthCheckerFactory ? settings.healthCheckerFactory(this) : null
  ...
  handleAuthorizedMessage(message) {
    switch(message.type) {
      ...
      case 'ping':
        ...
        if(this.healthChecker) this.healthChecker.handlePing(message)
        break;
      case 'pong':
        if(this.healthChecker) this.healthChecker.handlePong(message)
        break;
      case 'timeSync':
        let now = Date.now()
  ...
  handleMessage(message) {
    if (!this.dao && !this.daoPromise) {
      ...
      let daoPromise = this.daoFactory(message.sessionId, this.connection, this)
      if(!daoPromise.then) {
        this.dao = daoPromise
        daoPromise = null
      } else {
...
class ReactiveServer {
  ...
  handleConnection(connection) {
    ...
    let connection = new ReactiveServerConnection(this, id, connection, this.daoFactory, this.settings)
