class TimeSynchronization {
  constructor(settings) {
    ...
    */
    ...
    this.sendIntervalIncrement = settings.pingIntervalIncrement || 250
    this.maxSendInterval = settings.maxPingInterval || 10000
  ...
  handleTimeSync(clientTs, serverTs) {
    ...
    this.allTripsCount++
    if(this.allTripsCount == this.minPongCount) this.promiseCallback(this.timeDiff)

    //if(this.allTripsCount==5) console.error("TIME DIFF",this.minimalDiff,this.timeDiff,this.maximalDiff)
  }
  ...
  run() {
    ...
    let interval = this.sendInterval + this.allTripsCount * this.sendIntervalIncrement
    if(interval > this.maxSendInterval) interval = this.maxSendInterval
    setTimeout(() => this.run(), interval)
  }

  serverToLocal(ts) {
    if(this.allTripsCount < this.minPongCount) throw new Error("Time not synchronized")
