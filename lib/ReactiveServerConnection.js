...
class PushObservableTrigger {
  ...
  commitPointersUpdate(added, removed) {
    ...
    for(let n of added) {
      if(this.triggers.contains(n)) throw new Error("could not replace existing trigger")
  ...
  dispose() {
    ...
    for(let trigger of this.triggers.keys()) {
      trigger.dispose()
    }
...
class PushObservable extends ObservableList {
  ...
  pushObservation(what) {
    ...
    if(!observationInfo) {
      ...
      this.observations.push(observationInfo)
      this.push(what)
    }
