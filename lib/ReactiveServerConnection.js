...
class PushObservable extends ObservableList {
  constructor(connection, paths) {
    ...
    for(let path of paths) {
      this.triggers.push(new PushObservableTrigger(this, path.what, path.more))
    }
  }
...
class Observation {
  ...
  observe() {
    if(this.disposed) {
      ...
      throw new Error("Observe of disposed observation")
    }
    this.observed = true
  }
  unobserve() {
    if(!this.observed) throw new Error("Unobserve of not observed")
    this.observed = false
    if(this.pushCount == 0) this.dispose()
  }
  unpush() {
    this.pushCount--
    if(this.pushCount == 0) {
...
class ReactiveServerConnection extends EventEmitter {
  ...
  handleGetMore(requestId, paths) {
    ...
    function fetchDeps(source, schema) {
      ...
      console.log("S PTRS", pointers)
      if(moreDeps.length == 0) return Promise.resolve(pointers)
      return Promise.all(moreDeps.map(dep => fetch({ what: dep })))
    ...
    function fetch(path) {
      if(path.what) {
        ...
        return dataPromise.then(result => {
          ...
          if(path.more) {
            return fetchMore(result, path.more)
          } else return Promise.resolve()
        })
      } else if(path.schema) {
        return fetchDeps(undefined, path.schema).then(pointers => {
          console.log("PTRS", pointers)
          const results = pointers.map(pointer => fetch({ what: pointer }))
          if(path.more) {
            return fetchMore(results, path.more)
          } else return Promise.resolve()
        })
