...
class Observation {
  ...
  addObservable(observable) {
    console.log("ADD OBSERVABLE", this.what)
    if(this.disposed) throw new Error("observation use after disposal")
    this.observables.push(observable)
    observable.observation = this
    console.log("ADD OBSERVABLE observed: ", this.observed, "connected:", this.connection.connected)
    if(!this.observed && this.connection.connected) {
  ...
  removeObservable(observable) {
    ...
    if(this.connection.connected && this.observables.length == 0 && this.observed) {
      this.connection.sendUnobserve(this)
      this.observed = false
    }
    if(!this.observed && !this.pushed) {
...
class Connection extends EventEmitter {
  ...
  handleMessage(message) {
    ...
    if(message.type == "unpush") {
      ...
      if(!observation.observables.observer) {
  ...
  sendObserve(observation) {
    ...
    if(remoteObserves) {
      console.error("Observe duplication! existing observations",
          remoteObserves.map(({what, uid, disposed, observed }) => ({ what, uid, disposed, observed })),
          'new observation',
