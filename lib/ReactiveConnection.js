...
class Observation {
  ...
  removeObservable(observable) {
    ...
    if(this.connection.connected && this.observables.length == 0) this.connection.send({
      type: "unobserve",
      what: this.what,
      pushed: this.pushed
    })
...
class Connection extends EventEmitter {
  ...
  handleMessage(message) {
    ...
    if(message.type == "push") {
      ...
      if(observation) {
        ...
        this.observations.set(whatId, new Observation(this, message.what, true))
      }
    }
  ...
  removeObservation(what) {
    const whatId = JSON.stringify(what)
    let observation = this.observations.get(whatId)
    if(!observation) throw new Error(`observation ${whatId} not found`)
