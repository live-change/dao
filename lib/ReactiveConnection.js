...
class Observation {
  ...
  observable(generator) {
    ...
    observable.generator = generator
...
class Connection extends EventEmitter {
  ...
  sendObserve(observation) {
    //if(this.observes.has(JSON.stringify(observation.what))) throw "observe duplication "+observation.what
    this.observes.add(JSON.stringify(observation.what))
    this.send({
      type: "observe",
  ...
  sendUnobserve(observation) {
    //if(!this.observes.has(JSON.stringify(observation.what))) throw "unobserve duplication "+observation.what
    this.observes.delete(JSON.stringify(observation.what))
    this.send({
      type: "unobserve",
      what: observation.what,
      pushed: observation.pushed
