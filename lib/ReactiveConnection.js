...
class Connection extends EventEmitter {
  ...
  constructor(sessionId, settings) {
    ...
    this.healthChecker = settings.healthCheckerFactory ? settings.healthCheckerFactory(this) : null

    /// Backward compatibility TODO: remove in future
    if(this.settings.onDisconnect) this.on('disconnect', this.settings.onDisconnect)
    if(this.settings.onConnect) this.on('connect', this.settings.onConnect)
  }
  ...
  handleMessage(message) {
    if(message.type == "ping"){
      this.send({ type: "pong" })
      this.healthChecker.handlePing(message)
      this.emit('ping', message)
    }
    if(message.type == "pong"){
      this.healthChecker.handlePong(message)
